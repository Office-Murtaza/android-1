...
final class CoinExchangeViewController: ModuleViewController<CoinExchangePresenter> {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        presenter.didViewAppear.accept(())
    }
    ...
    func setupUIBindings() {
        ...
        presenter.alertShouldAppear.observeOn(MainScheduler.instance).subscribe { [weak self] _ in
            let alert = UIAlertController(title: nil, message: localize(L.Swap.Alert.coinsNotEnough), preferredStyle: .alert)
            let action = UIAlertAction(title: localize(L.Shared.ok),
                                       style: .default,
                                       handler: { [weak self] _ in self?.presenter.handleError() })
