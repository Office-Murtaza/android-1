...
protocol MainServiceWebSocket {
  ...
  func unsubscribeOrders() -> Completable
  func unsubscribeTrades() -> Completable
...
protocol OrdersSocketMethods {
  func getOrder() -> Observable<Order>
}

protocol TradeSocketMethods {
  func getTrade() -> Observable<Trade>
}

protocol MainSocketService: MainServiceWebSocket, BalanceSocketMethods, TransactionDetailsSocketMethods, OrdersSocketMethods, TradeSocketMethods {
}
...
class MainSocketServiceImpl: MainSocketService {
  ...
  @objc func handleBackground() {
    unsubscribeBalance()
      ...
      .andThen(unsubscribeOrders())
      .andThen(unsubscribeTrades())
      .andThen(disconnect())
      .subscribe()
      .disposed(by: disposeBag)
  }
