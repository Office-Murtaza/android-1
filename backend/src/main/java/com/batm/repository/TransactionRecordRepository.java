...
public interface TransactionRecordRepository extends JpaRepository<TransactionRecord, Long> {
    ...
    @Query("SELECT tr FROM TransactionRecord tr WHERE tr.cryptoCurrency IN :currency AND tr.tracked = false AND ((tr.type = 0 AND tr.status = 1) OR (tr.type = 1 AND tr.status = 3))")
    List<TransactionRecord> findUntrackedAndClosedTransactions(@Param("currency") Set<String> currency, Pageable page);
