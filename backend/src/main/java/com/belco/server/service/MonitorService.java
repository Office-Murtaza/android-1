...
//        coinNodeMap.put(CoinType.BITCOINCASH, new HashSet<>(Arrays.asList(bchNodeMainUrl)));
...
//        coinNodeMap.put(CoinType.DOGECOIN, new HashSet<>(Arrays.asList(dogeNodeMainUrl)));
    ...
    private boolean init(CoinType coinType) {
        ...
        if (!coinCurrentNodeMap.containsKey(coinType)) throw new RuntimeException("There is no node for " + coinType.name());
