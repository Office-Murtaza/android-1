...
class DbCryptoCoinModel : DbCryptoCoinInterface {
    override fun addCoin(realm: Realm, coinDb: DbCryptoCoin): Boolean = try {
    ...
    override fun delCryptoCoin(realm: Realm, _ID: Int): Boolean = try {
        realm.beginTransaction()
    ...
    override fun editCryptoCoin(realm: Realm, coinDb: DbCryptoCoin): Boolean = try {
        realm.beginTransaction()
        realm.copyToRealmOrUpdate(coinDb)
        realm.commitTransaction()
        true
    ...
    override fun getCryptoCoin(realm: Realm, coinId: Int): DbCryptoCoin? =
        realm.where(DbCryptoCoin::class.java).equalTo("_ID", coinId).findFirst()

    override fun getCryptoCoin(realm: Realm, coinType: String): DbCryptoCoin? =
        realm.where(DbCryptoCoin::class.java).equalTo("coinType", coinType).findFirst()

    override fun getAllCryptoCoin(realm: Realm): ArrayList<DbCryptoCoin> {
        val list = ArrayList<DbCryptoCoin>()
        val results = realm.where(DbCryptoCoin::class.java).findAll()
    ...
    override fun delAllCryptoCoin(realm: Realm): Boolean = try {
        realm.beginTransaction()
